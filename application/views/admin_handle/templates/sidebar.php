<!-- Sidebar -->
<div id="sidebar-wrapper">
    <ul class="sidebar-nav">
        <li class="sidebar-brand">
        </li>
        <li>
            <div class="box-profil">
                <img src="<?php echo base_url("assets/images/admin.png"); ?>" alt="..." class="img-circle " width="110" height="110"><br>
                <p class="nama_profile">Administrator</p>
            </div>
        </li>
        <li>
            <a href="<?php echo base_url(); ?>index.php/admin"> <span class="glyphicon glyphicon-home" aria-hidden="true"></span> Dashboard</a>
        </li>
        <li>
            <a href="<?php echo base_url(); ?>index.php/admin/list_data_barang"><span class="glyphicon glyphicon-th-list" aria-hidden="true"></span> List Data Barang</a>
        </li>
        <li>
            <a href="<?php echo base_url(); ?>index.php/admin/form_tambah_barang"><span class="glyphicon glyphicon-tasks" aria-hidden="true"></span> Data Barang</a>
        </li>
        <li>
            <a href="<?php echo base_url(); ?>index.php/admin/data_member"><span class="glyphicon glyphicon-tower" aria-hidden="true"></span> Data Member</a>
        </li>
        <li>
            <a href="<?php echo base_url(); ?>index.php/admin/data_transaksi"><span class="glyphicon glyphicon-transfer" aria-hidden="true"></span>Data Transakasi</a>
        </li>
        <li>
            <a href="<?php echo base_url(); ?>index.php/admin/kategori_barang"><span class="glyphicon glyphicon-list-alt" aria-hidden="true"></span>Kategori Barang</a>
        </li>
        <li>
            <a href="<?php echo base_url(); ?>index.php/admin/stok_barang"><span class="glyphicon glyphicon-qrcode" aria-hidden="true"></span>Stok Barang</a>
        </li>
        <li>
            <a href="<?php echo base_url(); ?>index.php/admin/profil_admin"><span class="glyphicon glyphicon-user" aria-hidden="true"></span>Profil</a>
        </li>
        <li>
            <a href="#"><span class="glyphicon glyphicon-off" aria-hidden="true"></span>Logout</a>
        </li>
    </ul>
</div>
<!-- End sidebar-wrapper -->
